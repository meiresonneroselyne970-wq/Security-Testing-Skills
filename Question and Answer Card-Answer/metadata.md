...
精简 AI 知识问答结果卡片模板，专注回答展示。无头部、无快捷提问、无 API 健康检测，配置硬编码在 JS 中。适用场景：嵌入已有页面、仅展示问答结果。
...
├── index.html       # 页面结构（卡片骨架 + 外部输入栏 + 加载 CSS/JS）
├── style.css        # 卡片样式（渐变装饰条 + 文件类型色标 + 动画）
├── app.js           # 问答逻辑（API 调用 + 状态管理 + DOM 渲染，配置硬编码）
└── metadata.md      # 本文件
```
...
body
  ...
  │           │     └── [qa-error]              ← 服务端错误块（可选）
  │           └── [qa-state]                   ← 空结果状态
  └── .input-row#qaFooter                      ← 输入行（卡片外部，独立元素）
        ├── input#qaInput                      ← 文本输入框（独立阴影 + 聚焦光晕）
        └── button.send-btn                    ← 发送按钮（紫蓝渐变圆形，hover 放大）
```
...
| 输入行位置 | 卡片内部 footer | 卡片外部，独立元素 |
| 背景光晕 | ✅ 有（.bg-layer fixed） | ❌ 无（body 上的 radial-gradient） |
...
| 加载中 | 点击发送/Enter，等待 API 响应 | `.typing-dots`（三个 10px 跳动圆点） |
| 回答 | API 返回成功 | `.qa-answer`（AI 头像 + 正文 + 来源文件） |
| 空结果 | API 返回但无有效内容 | `.qa-state`（🤔 + "未找到相关内容"） |
| 错误 | 网络错误 / 超时 | `.qa-error`（⚠️ 粉红背景 + 错误详情） |
...
7. **输入行外置**：`.input-row` 在卡片外部渲染，作为独立元素。注释标注"临时输入窗口（后期删掉，上传到服务器）"，表明此为开发阶段的测试入口
8. **发送防抖**：`isSending` 标志位防止重复提交，输入框和按钮同步 disabled
9. **纯前端**：无框架依赖，零构建步骤，可直接在浏览器中打开
